...
export const buildFallbackAnalysis = (metrics, locale = 'en') => {
    ...
    if (riskLevel === 'high') {
        return {
            ...
            pathology_summary: isChinese
                ? `患者 TIR 为 ${metrics.tir}%，CV 为 ${metrics.cv}%，TAR 为 ${metrics.tar}%。结果提示血糖波动和高血糖暴露偏高，可能增加氧化应激、内皮功能障碍、视网膜微血管渗漏以及早期肾脏微血管损伤风险。建议结合真实病史、用药和餐后曲线进一步复核。`
                : `Patient TIR is ${metrics.tir}% with CV ${metrics.cv}% and TAR ${metrics.tar}%. The pattern suggests elevated glycemic variability and hyperglycemic exposure, which may increase oxidative stress, endothelial dysfunction, retinal microvascular leakage, and early nephropathy risk. Review this with medication, meals, and clinical history.`,
